...
import { Note, NoteFileData, NoteInput } from "./index.js"
...
export async function uploadNewNotes({
  ...
  const { data, error } = await apiClient.upsertNotes(newNotes)
  if (error) {
    throw error
  }
  ...
  const { error: error2 } = await apiClient.upsertNoteFiles(noteFileData)
  if (error2) {
    throw error2
  }
}
